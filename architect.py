...
class Architect():
    ...
    def compute_hessian(self, dw, trn_X, trn_y):
        ...
        norm = paddle.concat([w.view(-1) for w in dw]).norm()
        eps = 0.01 / norm
        ...
        dalpha_pos = paddle.fluid.backward.gradients(loss, self.net.alphas()) # dalpha { L_trn(w+) }
        ...
        dalpha_neg = paddle.fluid.backward.gradients(loss, self.net.alphas()) # dalpha { L_trn(w-) }
    ...
    def compute_eigenvalues(self):
        ...
        for t in self.hessian :
            if i == 0:
                mm=t.cuda().data.cpu().numpy()
                i=1
            else:
                mm = np.row_stack((mm, t.cuda().data.cpu().numpy()))
        print(mm.shape)
        return eigvals(mm)
